...
public class CondorSpread extends AbstractSpread {
	...
	order.addPosition(new Position(getTradeId(), -quantity, symbol, expiry, strikeList.get(2), type));
	order.addPosition(new Position(getTradeId(), quantity, symbol, expiry, strikeList.get(3), type));
